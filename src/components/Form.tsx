...
function Form() {
  ...
  function handelAddNote(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
    if(textareraRef.current !== null && textareraRef.current.value ) {
  ...
  return (
    <form className='flex flex-col' onSubmit={handelAddNote} >
        <div className='relative inline-block' >
          ...
          <div className='absolute top-0 left-0 w-[30rem] h-[20rem] -z-10 '  dangerouslySetInnerHTML={{ __html: value }}></div>
        </div>
